...
import 'schema/business_partner_record.dart';
...
export 'schema/business_partner_record.dart';
export 'schema/products_record.dart';
export 'schema/invoice_record.dart';
export 'schema/cart_record.dart';
export 'schema/order_record.dart';

/// Functions to query BusinessPartnerRecords (as a Stream and as a Future).
Future<int> queryBusinessPartnerRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      BusinessPartnerRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<BusinessPartnerRecord>> queryBusinessPartnerRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
    ...
    queryCollection(
      BusinessPartnerRecord.collection,
      BusinessPartnerRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<BusinessPartnerRecord>> queryBusinessPartnerRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future maybeCreateUser(User user) async {
  final userRecord = BusinessPartnerRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument =
        await BusinessPartnerRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createBusinessPartnerRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      BusinessPartnerRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createBusinessPartnerRecordData(email: email));
}
